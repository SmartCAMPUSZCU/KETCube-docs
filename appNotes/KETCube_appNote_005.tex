...
\begin{itemize}
	\item \texttt{length} - [8b] sum of the \texttt{parameter payload} and the \texttt{command tree path} lengths
		\begin{itemize}
			\item \sout{excludes fixed-size common header and self}
		\end{itemize}
	\item \texttt{module ID} - [8b or 16b] module ID\sout{, unused for now}
		\begin{itemize}
			\item \sout{reserved, set to 0 for all batch packets}
		\end{itemize}
	\item \texttt{parameter payload} - [variable length] serialized parameters of a given command
		\begin{itemize}
			\item if the command does not require any parameters, this field is missing
...
\sout{The payload structure is repeated for each batch command.}
