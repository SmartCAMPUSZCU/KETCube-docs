...
\begin{versionhistory}
  \vhEntry{0.2*}{27.5.2022}{JB|KV}{Initial internal version}
\end{versionhistory}
...
\node[draw,
    ...
    minimum height=1cm,
    align=center
] (block0) { Init and SelfCheck\\Start Bootloader };
...
\node[draw,
    ...
    minimum height=1cm,
    align=center
] (block1) { 0x7F or command word\\received in the time window\footnotemark\\on one of the\\supported interfaces\footnotemark };
...
\node[draw,
    ...
    minimum height=1cm,
    align=center
] (block2) { USART type peripheral\\selected };
...
\node[draw,
    ...
    minimum height=3cm,
    align=center
] (block3) { For the auto-baud-rate\\send ACK byte\footnotemark };
...
\node[draw,
    ...
    minimum height=3cm,
    align=center
] (blockCmdWait) { Disable unused\\peripherals\addtocounter{footnote}{-2}\footnotemark\addtocounter{footnote}{+1}};
...
\node[draw,
    below=2cm of blockCmdRecv,
    minimum width=3cm,
    minimum height=2cm,
    align=center
] (block6) { Basic or Extended\\Commands\\(Get, Read, Write, \dots) };
...
\node[draw,
    ...
    minimum height=2cm,
    align=center
] (block7) { GO command:\\valid and\\enabled address? };
...
\node[coordinate,right=1cm of block7] (blockGoNo) {};
...
\draw[->] (block1) -| (block3b) node [pos=0.1,above,font=\footnotesize] { No };

\draw[->] (block3b) |- (blockCmdWait) node [pos=0.3,left,font=\footnotesize] { No };
\draw[->] (block3b) -| (blockJumpApp) node [pos=0.1,above,font=\footnotesize] { Yes };
              
\draw[->] (blockCmdRecv) -| (block6);
\draw[->] (blockCmdRecv) -| (block7);

\draw[] (block6) |- (blockCmdExec);
\draw[->] (block7) -- (blockJumpAddr) node [pos=0.2,right,font=\footnotesize] { Yes };
\draw[-] (block7) -| (blockGoNo) node [pos=0.1,above,font=\footnotesize] { No };
\draw[->] (blockGoNo) |- (block4);

\draw[] (blockCmdExec) |- (blockCmdExec2)
        (blockCmdExec2) |- (blockCmdExec3);
\draw[->] (blockCmdExec3) |- (block4);
 
\end{tikzpicture}}}
\addtocounter{footnote}{-2}
\footnotetext{The dafault time window for 0x7F reception is 5 seconds}
\addtocounter{footnote}{+1}
\footnotetext{USART only is currently supported}
\addtocounter{footnote}{+1}
...
The Bootloader API object (structure of function pointers) is located at the beginning of the (Bootloader) RAM area in the \texttt{.api} section to allow straightforward application-bootloader interfacing. 
...
\docNote{Application \textbf{should not} use any memory reserved for bootloader! 

RAM used by the bootloader can be re-used by the application if and only if the application does not use the bootloader API.}
...
\clearpage
 ...
 \docNote{Note, that there is no need to reset the fragment version field or watch for overflow because the 32-bit version field allows representing more than 10M of full 6kB EEPROM writes (in the pessimistic case, for the smallest possible fragments), which is much above typical EEPROM durability.}
...
\docWarn{If an EEPROM bank is permanently damaged, the current implementation in the single-bank case is unable to recover - no new patches could be stored.
...
To add a new device:
\begin{enumerate}
\item see the \docFileName{Targets} directory, add a new target and implement the startup file, interrupt handler, and the custom target Makefile if needed,
\item insert the device definitions to the \docFileName{device.h} file, 
\item re-visit or modify \docFileName{device.c} file and the target speciffic (\docVarName{device.c} file,
...
\item re-visit or modify the interface implementation(s)\\in~\docFileName{iface\_IFACE.c},
...
\docNote{To select the device, set \docVarName{DEVICE} variable in the Makefile to the device-speciffic string, e.g. to \docVarName{STM32L071xx} to select STM32l071xx device.}
...
\marginlabel{\captionof{figure}{Bootloader Binary Image Format}\label{fig:arch:MemImg}}
\raisebox{-\height}{

\begin{tikzpicture}
  \node[draw,fill=SeaGreen3,minimum height=6cm,minimum width = 5cm,yshift=3cm](10/5){Bootloader Image};
  \node[draw,fill=BlueGreen!30!,    minimum height=1cm,minimum width = 5cm,yshift=1cm](10/5){ Image CRC };
  \node[draw,fill=BlueGreen!30!,    minimum height=1cm,minimum width = 5cm,yshift=0cm](10/5){ Image Length in bytes};
  \node[draw,fill=white,    minimum height=2cm,minimum width = 5cm,yshift=-1.5cm](10/5){ zero padding };
  
  \node[anchor=center,yshift=-3.0cm,xshift=0cm,fill=white] { Bootloader Binary Image };
\end{tikzpicture}
}

The bootloader image should be created in a way analogous to the recommended following process:
...
\subsection{Custom Build Configuration}
  ...
  \begin{table*}[!ht]
    ...
    \begin{tabular}{| p{5.5cm} | p{7.5cm} |}
        ...
        \hline
        \texttt{RNG\_USE\_FALLBACK\_TRNG} & Use fallback time-seeded PRNG \\
        \hline
...
\begin{itemize}
  \item \texttt{Locked State} -- in the locked state, access privileges -- see \nameref{sec:security:secModes} below -- are used to control access to bootloader functions.
...
\docNote{For privileges lower than \docVarName{ACCESS\_PRIV\_CONFIG}, the new application could be uploaded to the application area, however, it could not be executed until it is verified by AES-CMAC. Unsigned applications could not be executed!}

\docNote{For privileges lower than \docVarName{ACCESS\_PRIV\_CONFIG}, the bootloader configuration is written and read-out protected thru the bootloader protocol (not from the running application).}
...
\begin{itemize}
  \item modify \docVarName{VTOR}\footnote{\docVarName{Vector Table Offset Register} -- defines the offset of the interrupt vector table in the ARM memory} register -- e.g. applications generated by STM32Cube set the \docVarName{VTOR} register to the beginnig of the flash area if\\\docVarName{USER\_VECT\_TAB\_ADDRESS} is defined
...
\clearpage
\subsection{Application Programming}\label{sec:app:prog}
The application image is the plain flash image (code and data).
...
$ ./stm32flash -w build/application.bin -S 0x8004000 /dev/ttyUSB0
stm32flash 0.5
...
Write to memory
Erasing memory
Wrote and verified address 0x08004ebc (100.00%) Done.
...
$ ADDR="08004000"
$ ./stm32flash -x 0140:${ADDR}${LEN}${CRC} /dev/ttyUSB0
stm32flash 0.5
...
\docNote{By default, the bootloader checks the CRC of the application image in the specified range -- without setting the application start address, application length, and CRC, the application will not start! The only exception is when the application is successfully verified by CMAC, then application settings are set automatically and there is no need for the custom setup -- see \ref{sec:app:verify}}

\clearpage
\subsection{Application Verification}\label{sec:app:verify}
To verify the application uploaded to MCU's memory, the CMAC signature using the correct CMAC key must be generated.

\docWarn{The application verification by CMAC is required when user rights are lower than \docVarName{ACCESS\_PRIV\_CONFIG}. The user with \docVarName{ACCESS\_PRIV\_WRITE} access priviledges must upload the application CMAC to allow application execution. For detail, please see \ref{sec:security:secModes}.}

\docNote{Please note, that currently, only the signature of the whole flash is supported by the Bootloader, thus CMAC must be computed for whole memory, where the firmware image is padded by zeroes to the full flash size.}

The following setup is recommended to generate firmware signature and upload the signed firmware to the MCU's memory:
...
# How many bytes left in the memory when your firmware is uploaded
PADDING=$(( 180224 - $( du -b ${FW} | awk '{print $1}' ) ))

# Create temporary firmware file padded to full-size memory
cp ${FW} ${FW}~
truncate -s +${PADDING} ${FW}~
...
# Erase MCU memory
$ ./stm32flash -S 0x8004000:180224 -o ${INTERFACE}

...

# Upload the original (smaller) FW image
$ ./stm32flash -w ${FW} -S 0x8004000 ${INTERFACE}

...

# Upload CMAC of the Flash to verify image in the memory
$ ./stm32flash -x 0130:${CMAC}+60 ${INTERFACE}

...

# Start your uploaded and verified firmware
$ ./stm32flash -g 0x8004000
...
Starting execution at address 0x08004000... done.
\end{verbatim}
\end{docCodeExampleTitled}

\docNote{When application CMAC is uploaded and successfully verified, application settings (the application start address, application length, and the application CRC) are set automatically and the application si ready to start. The application length and CRC is however set concerning the whole flash application area: for application startup optimization, you can set the parameters manually, as described in \ref{sec:app:prog} (if enabled by your access rights -- \docVarName{ACCESS\_PRIV\_CONFIG} is required)}
...
If the bootloader is set to alow flash uploads of unencrypted (MAC-verified) code,
the code is first uploaded to the memory and the signature verification follows.
This is a standard behavior, however, signature verification is required to allow firmware to boot.

BUT, the unverified code is present in the program memory and could be executed using the \docVarName{go} command from the ROM bootloader.

Unverified code execution allows full access to the MCU memories.

\subsubsection*{Countermeasures}

The portable countermeasure included in the bootloader is the memory obfuscation. 
When unencrypted code is uploaded to the bootloader memory in the security access mode lower than (see \ref{sec:security:secModes})
\docVarName{ACCESS\_PRIV\_CONFIG}, each word uploaded to the memory is obfuscated by a random mask number leading to the invalid code in the MCUs' memory.
The code is de-obfuscated after successfull MAC verification. 
Please note, that the mask is lost when MCU reset, thus the MAC verification must follow application code write.

The other countermeasure option is not portable in general, 
but could be included for targets supporting the Memory Protection Unit (MPU) 
allowing disable code execution from the specific address range. 
This countermeasure is not portable, however it speeds up the code upload process.

\docNote{For targets, where MPU is not present, or MPU support is not included in the bootloader, 
the bootloader will automatically fallback into the obfuscation countermeasure scheme.}

\docWarn{Please note, that MPU support was not implemented yet even for default targets!}
...
\begin{table*}[!ht]
  ...
  \begin{tabular}{| p{3cm} | p{1.5cm} | p{1cm} | p{1cm} | p{1cm} | p{4.5cm} | }
      ...
      \hline
      Write Unlock Response & \texttt{0x0121} & 16 & 0 & 0 & Write Unlock Seed encrypted by the Unlock Key\\
      \hline
      Unlock Password & \texttt{0x0122} & 0 & 16 & 0 & Unlock Bootloader by password\\
      \hline
      Lock & \texttt{0x0123} & 0 & 0 & 0 & Lock Bootloader\\
      \hline
      Verify Flash CMAC & \texttt{0x0130} & 16 & 0 & 0 & Write application CMAC to verify the application memory signature; \texttt{0x01} is returned in the status field when success; if success, application start address is set to the application flash begin as a side-effect\\
      \hline
      Set Application & \texttt{0x0140} & 12 & 0 & 0 & Write application settings: 3 32-bit words (concatenation of three Big-Endian integers) representing application start address, application image length and its CRC\\
      \hline
  \end{tabular}
  \addcontentsline{lot}{table}{Bootloader Special Commands}
  \label{tab:specCmd}
...
\docNote{The test-build-only special commands are not documented here. 

See the Doxygen-generated code-level documentation for details.}
