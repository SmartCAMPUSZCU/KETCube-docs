...
\clearpage
\subsection{Bootloader Module Dependencies}
...
\subsection{Bootloader Configuration}
  The primary clock source is PLLCLK at 32 MHz, derived from HSI.
...
\clearpage
\subsection{Program Flash Memory Map} \label{sec:arch:flashMap}
...
\clearpage
\subsection{Non-Volatile Data Memory Map -- Secure Storage} \label{sec:arch:EEPROMMap}

The microcontroller Non-Volatile Memory is typically organized in one or more banks.
The typical EEPROM memory organization with two banks is shown in Figure~\ref{fig:arch:EEPROMMap}.

\docNote{Our default targets (\texttt{STM32l0xx}) are quit typical and employ two EEPROM Banks.}

\marginlabel{\captionof{figure}{Physical\\EEPROM Organization\\(STM32l082)}\label{fig:arch:EEPROMMap}}
\raisebox{-\height}{

\begin{tikzpicture}
  \node[draw,fill=SeaGreen1,    minimum height=3cm,minimum width = 5cm,yshift=0cm](10/5){Bank 1};
  \node[draw,fill=SeaGreen3,minimum height=3cm,minimum width = 5cm,yshift=3cm](10/5){Bank 0};
  ...
  \node[anchor=center,yshift=-2.0cm,xshift=0cm,fill=white] {EEPROM Data Memory};
\end{tikzpicture}
}
...
\marginlabel{\captionof{figure}{Logical NVM Organization}\label{fig:arch:Block}}
\raisebox{-\height}{
...
\clearpage
 ...
 \docNote{Note, that there is no need to reset the fragment version field or watch for overflow, because 32-bit version field allows to represent more than 10M of full EEPROM writes (in the pessimistic case, for the smallest possible fragments), which is much above typical EEPROM durability.}
...
\clearpage
\subsection{Bootloader States} \label{sec:arch:states}
...
\docNote{TODO: state transition diagram.}

\subsection{Bootloader Security Modes} \label{sec:arch:secModes}

\begin{itemize}
  \item \texttt{No protection} -- 0x0000
  \item \texttt{Readout Protection} -- 0x0001
  \item \texttt{Bootloader Lock} -- 0x0002
\end{itemize}

\begin{docCodeSnippetTitled}{Security Mode Transition Algorithm}
\begin{verbatim}
...
\begin{table*}[!ht]
  ...
  \begin{tabular}{| p{4cm} | p{9cm} | }
      ...
      \hline
      {\tt security\_t } \newline ({\tt uint16\_t }) & {\tt 0x0000} -- only commands needed for bootloader configuration unlock are enabled;\newline
                           {\tt 0x0001} -- application code/data memory readout protection, bootloader configuration is key-locked, INFO commands enabled;\newline
                           {\tt 0x0002} -- application code/data memory readout protection, bootloader configuration is key-locked, memory write enabled;\newline
                           {\tt 0x0003} -- application code/data memory readout enabled, bootloader configuration is key-locked;\newline
                           {\tt 0x0004} -- application code/data memory readout enabled, bootloader configuration is unlocked;\newline
                           {\tt 0x0005} -- application code/data memory readout enabled, bootloader configuration is unlocked, test commands are enabled.\newline
                           \begin{tabular}{cc|c|c|c|c|c}
                           	& 
                           	\rotatebox[origin=c]{90}{\parbox{3cm}{~key unlock}} &
                           	\rotatebox[origin=c]{90}{\parbox{3cm}{~info commands}} &
                           	\rotatebox[origin=c]{90}{\parbox{3cm}{\centering application code/\\data memory\\ write}} &
                           	\rotatebox[origin=c]{90}{\parbox{3cm}{\centering application code/\\data memory\\ readout}} &
                           	\rotatebox[origin=c]{90}{\parbox{3cm}{~bootloader\\configuration}} &
                           	\rotatebox[origin=c]{90}{\parbox{3cm}{~test commands}}
                           	\\\hline
                           	 0x0000	&Y&&&&&\\\hline
                            0x0001	&Y&Y&&&&\\\hline
							0x0002	&Y&Y&Y&&&\\\hline
							0x0003	&Y&Y&Y&Y&&\\\hline
							0x0004	&Y&Y&Y&Y&Y&\\\hline
							0x0005	&Y&Y&Y&Y&Y&Y\\
                           \end{tabular}\\
      \hline
...
\subsection{Bootloader Build Options}
  ...
  \begin{table*}[!ht]
    ...
    \begin{tabular}{| p{5.5cm} | p{7.5cm} |}
        ...
        \hline
        \hline
        \texttt{IFACE\_BR\_115200} & UART interface baudRate is 115200\\
        \hline
        \texttt{IFACE\_PARITY\_NONE} & UART interface parity is NONE\\
        \hline
        \texttt{TESTS\_ENABLE} & All test commands are linked and enabled\\
        \hline
