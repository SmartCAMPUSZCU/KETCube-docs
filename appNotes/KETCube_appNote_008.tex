...
\subsection{Custom Build Configuration}\label{sec:arch:CustBuildConfig}
  ...
  \begin{table*}[!ht]
    ...
    \label{tab:cmdset}
   \end{table*}
   \footnotetext{See note in \ref{sec:vln:001}}
...
If the acccess rights are set to \docVarName{ACCESS\_PRIV\_CONFIG}, or unverified code uploads are allowed -- \docVarName{integrity\_t} is set to \docVarName{0x0000}, and \docVarName{ACCESS\_PRIV\_WRITE} is set, the plain application image could be uploaded.

In this simple case, the application image is the plain flash image (code and data) and should be uploaded as follows:
...
If access rights are set to \docVarName{ACCESS\_PRIV\_CONFIG},
...
If access rights are lower than \docVarName{ACCESS\_PRIV\_CONFIG},
but \docVarName{integrity\_t} is set to \docVarName{0x0000},
the code verification command must be executed with arbitrary (e.g. all-zero) MAC:
...
\clearpage
\subsection{Signed Application Programming}\label{sec:app:verify}
To verify the application uploaded to MCU's memory, the CMAC signature using the correct CMAC key must be generated.

\docWarn{The application verification by CMAC is required when user rights are lower than \docVarName{ACCESS\_PRIV\_CONFIG} and the \docVarName{integrity\_t} is set to \docVarName{0x0001}, \docVarName{0x0002} or \docVarName{0x0003} (see \ref{sec:cfg:runtime}). The user with \docVarName{ACCESS\_PRIV\_WRITE} access priviledges must upload the application CMAC to allow application execution. For detail, please see \ref{sec:security:secModes}.}

The following setup is recommended to generate firmware signature and upload the signed firmware to the MCU's memory:

\begin{docCodeExampleTitled}{The Bootloader Appliaction Upload Script}
\begin{verbatim}
...
\docNote{When application CMAC is uploaded and successfully verified, application settings (the application start address, application length, and the application CRC) are set automatically and the application is ready to start.}
...
BUT prior to verification, the unverified code is present in the program memory and could be executed using the \docVarName{go} command from the ROM bootloader.

Unverified code execution allows full access to the MCU memories and right escalation up to the \docVarName{ACCESS\_PRIV\_CONFIG} (see \ref{sec:security:secModes}).
...
for access modes lower than \docVarName{ACCESS\_PRIV\_CONFIG}.
...
\docWarn{Still small portions of unencrypted code may occur in the MCU RAM in the bootloader protocol Rx buffer. 
This is unfortunately unavoidable without MPU, while this recommendation lowers the risk significantly.}

\subsubsection*{Countermeasures}

The protection implemented to avoid secret leakage is, that sensitive data are erased from the MCU memory if the access mode is lower than  \docVarName{ACCESS\_PRIV\_CONFIG}, MPU is not present, and an allowed write attempt of the unencrypted code image occurs (access mode is set to \docVarName{ACCESS\_PRIV\_WRITE}). This protection could be disabled by setting the build config variable \docVarName{ACCESS\_ENABLE\_INSECURE\_IMG} (see Section \ref{sec:arch:CustBuildConfig}).
...
\docWarn{Please note, that MPU support was not implemented yet, as it is not supported by the default target!}
