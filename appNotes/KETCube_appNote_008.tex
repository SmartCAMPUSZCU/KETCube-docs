...
\begin{versionhistory}
  \vhEntry{0.2*}{27.5.2022}{JB|KV}{Initial version}
\end{versionhistory}
...
\docNote{RAM used by the bootloader cannot be re-used by the application. 
...
\clearpage
\subsection{Bootloader Binary Image}
The application image is the plain flash image (code and data) extended by a minimal set of metadata needed to perform the code self-check.
The metadata is prefixed to the image and compose a custom image header.

When flashing the image to the MCU flash, ensure, 
that the image is prefixed by two 32-bit Little-endian words meaning the CRC-32 of the image (excluding the header), 
and the image size (excluding the header), which is always non-zero.

\marginlabel{\captionof{figure}{Application Binary Image Format}\label{fig:arch:MemImg}}
\raisebox{-\height}{

\begin{tikzpicture}
  \node[draw,fill=BlueGreen!30!,    minimum height=1cm,minimum width = 5cm,yshift=3cm](10/5){ Image CRC };
  \node[draw,fill=BlueGreen!30!,    minimum height=1cm,minimum width = 5cm,yshift=2cm](10/5){ Image Length in bytes};
  \node[draw,fill=SeaGreen3,minimum height=6cm,minimum width = 5cm,yshift=-1.5cm](10/5){Application Image};
  
  \node[anchor=center,yshift=-5.0cm,xshift=0cm,fill=white] { Application Binary Image };
\end{tikzpicture}
}

The application image should be created in a way analogous to the recommended following process:

\begin{docCodeExampleTitled}{Bootloader Image Creation Process}
\begin{verbatim}
$ arm-none-eabi-objcopy -O binary -S build/application.elf \
    build/application.bin
$ gzip build/application.bin -c | tail -c8 \
    > build/application.bin_header
$ cat build/application.bin_header > build/application.bin~
$ cat build/application.bin >> build/application.bin~
$ mv build/application.bin~ build/application.bin
\end{verbatim}
\end{docCodeExampleTitled}
...
If the bootloader is set to alow flash uploads of unencrypted signed code only,
the unsigned code could be uploaded without signature verification.
This is a standard behavior, however, signature verification is required to allow firmware boot.

BUT, the unverified code could be executed using the \docVarName{go} command in the ROM bootloader.
